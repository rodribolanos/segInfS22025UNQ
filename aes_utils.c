...
unsigned char inv_mix_col_matrix[BLOCK_DIM][BLOCK_DIM] = {
    { 0x0E, 0x0B, 0x0D, 0x09 },
    { 0x09, 0x0E, 0x0B, 0x0D },
    { 0x0D, 0x09, 0x0E, 0x0B },
    { 0x0B, 0x0D, 0x09, 0x0E }
...
// addRoundKey es lo mismo solo que va de 15 a 0 

void inv_shiftRows(unsigned char state[BLOCK_DIM][BLOCK_DIM]){
    // rotacion a la derecha;
    for(int i = 1; i < BLOCK_DIM; i++)
        rightRotate(state[i], i, BLOCK_DIM);
}
...
void inv_mixColumns(unsigned char state[BLOCK_DIM][BLOCK_DIM]){
    ...
    for(int i = 0; i < BLOCK_DIM; i++){
        for(int j = 0; j < BLOCK_DIM; j++){
            ...
            for(int h = 0; h < BLOCK_DIM; h++){
                acc[i][j] ^= closedMult(inv_mix_col_matrix[i][h], state[h][j]);
            }
        }
    }
